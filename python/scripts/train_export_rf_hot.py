#!/usr/bin/env python3
"""
Train and export a hotness-heavy random-forest cache-admission model aligned
with the online C++ RF admission gate.

Outputs:
- final_model_params.json: exported forest metadata, feature order, hyperparams
- rf_hot_cache_admission_params.inc: C++ snippet for rf_hot_cache_admission.h
...
- feature_importances.csv: final fitted RF feature importances
...
DB_LABEL_COLUMN = "_db_label"

DEFAULT_DATA_ROOT = Path("rocksdb_exp/final_paper_matrix")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/export a hotness-heavy random-forest admission model."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("python/output/train_export_rf_hot"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel workers for RandomForest.",
    )
    parser.add_argument(
        "--random-state",
        type=int,
...
def _infer_rel_parts(dataset_path: Path, data_root: Path) -> Tuple[str, ...]:
    try:
        rel = dataset_path.resolve().relative_to(data_root.resolve())
        return rel.parts
    except Exception:
        return dataset_path.parts
...
def infer_db_label(dataset_path: Path, data_root: Path) -> str:
    parts = _infer_rel_parts(dataset_path, data_root)
...
def load_datasets(paths: Sequence[Path], data_root: Path) -> pd.DataFrame:
    ...
    for path in paths:
        ...
        df[DB_LABEL_COLUMN] = infer_db_label(path, data_root)
        frames.append(df)
...
def run_leave_one_run_out(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(logo.split(x, y, groups), start=1):
        ...
        fold_df = df.iloc[test_idx][
            [RUN_COLUMN, DATASET_COLUMN, WORKLOAD_COLUMN, DB_LABEL_COLUMN]
        ].copy()
    ...
    summary = {
        ...
        "num_db_labels": float(df[DB_LABEL_COLUMN].nunique()),
...
def export_params(pipe: Pipeline, feature_names: Sequence[str]) -> Dict[str, object]:
    ...
    for estimator in model.estimators_:
        ...
        for node_idx in range(tree.node_count):
            ...
            feature_index = -1 if is_leaf else int(tree.feature[node_idx])
            threshold = 0.0 if is_leaf else float(tree.threshold[node_idx])

            values = tree.value[node_idx][0]
            value0 = float(values[0]) if len(values) >= 1 else 0.0
            value1 = float(values[1]) if len(values) >= 2 else 0.0

            nodes.append(
                {
                    "feature_index": feature_index,
                    "left": -1 if is_leaf else base + left,
                    "right": -1 if is_leaf else base + right,
                    "threshold": threshold,
                    "value0": value0,
                    "value1": value1,
                    "is_leaf": is_leaf,
...
def format_cpp_tree_meta(trees: Sequence[Dict[str, int]], indent: str = "      ") -> str:
    lines = []
    for tree in trees:
        lines.append(f"{indent}{{{tree['root']}}},")
    if lines:
        lines[-1] = lines[-1].rstrip(",")
    return "\n".join(lines)


def format_cpp_nodes(nodes: Sequence[Dict[str, object]], indent: str = "      ") -> str:
    lines = []
    for node in nodes:
        lines.append(
            f"{indent}"
            "{"
            f"{node['feature_index']}, {node['left']}, {node['right']}, "
            f"{node['threshold']:.17g}, {node['value0']:.17g}, "
            f"{node['value1']:.17g}, {node['is_leaf']}"
            "},"
        )
    if lines:
        lines[-1] = lines[-1].rstrip(",")
...
def build_cpp_snippet(params: Dict[str, object]) -> str:
    ...
    return (
        "// Auto-generated by python/scripts/train_export_rf_hot.py\n"
        "// Feature order must match RFHotCacheAdmissionFeatures.\n"
        f"{comments}\n"
        f"  static constexpr size_t kRFHotNumFeatures = {len(features)};\n"
        f"  static constexpr size_t kRFHotNumTrees = {len(params['trees'])};\n\n"
        "  static constexpr RFHotTreeMeta kRFHotTrees[] = {\n"
        f"{format_cpp_tree_meta(params['trees'])}\n"
        "  };\n\n"
        "  static constexpr RFHotTreeNode kRFHotNodes[] = {\n"
        f"{format_cpp_nodes(params['nodes'])}\n"
        "  };\n"
    )
...
def write_report(
    ...
    with report_path.open("w", encoding="utf-8") as f:
        f.write("# RF Hot Admission Export Report\n\n")
        f.write("## Data\n\n")
        f.write(f"- datasets: `{len(dataset_files)}`\n")
        f.write(f"- db_labels: `{', '.join(db_labels)}`\n")
        ...
        f.write("## Model\n\n")
        f.write(f"- trees: `{len(params['trees'])}`\n")
        f.write(f"- nodes: `{len(params['nodes'])}`\n\n")
        f.write("## Top Features\n\n")
        for row in feature_importances.head(10).itertuples(index=False):
            f.write(f"- `{row.feature}`: `{row.importance:.6f}`\n")
        f.write("\n## Artifacts\n\n")
        f.write("- `final_model_params.json`\n")
        f.write("- `rf_hot_cache_admission_params.inc`\n")
        f.write("- `oof_predictions.csv`\n")
        f.write("- `threshold_metrics.csv`\n")
        f.write("- `feature_importances.csv`\n")
...
def main() -> None:
    ...
    to_numeric_if_present(df, RF_HOT_FEATURES)

    oof, summary = run_leave_one_run_out(df, RF_HOT_FEATURES, args)
    y_true = oof["y_true"].to_numpy()
    y_prob = oof["y_prob"].to_numpy()
    ...
    final_pipe = fit_final_model(df, RF_HOT_FEATURES, args)
    final_model: RandomForestClassifier = final_pipe.named_steps["model"]
    params = export_params(final_pipe, RF_HOT_FEATURES)

    with (output_dir / "final_model_params.json").open("w", encoding="utf-8") as f:
        json.dump(params, f, indent=2)
    (output_dir / "rf_hot_cache_admission_params.inc").write_text(
        build_cpp_snippet(params), encoding="utf-8"
    )

    feature_importances = pd.DataFrame(
        {
            "feature": RF_HOT_FEATURES,
    ...
    db_labels = sorted(df[DB_LABEL_COLUMN].unique().tolist())
    write_report(
        output_dir,
        summary,
        threshold_metrics,
    ...
    print(f"Wrote: {output_dir / 'rf_hot_cache_admission_params.inc'}")
    print(f"Wrote: {output_dir / 'oof_predictions.csv'}")
    print(f"Wrote: {output_dir / 'threshold_metrics.csv'}")
    print(f"Wrote: {output_dir / 'feature_importances.csv'}")
