...
# Batch online evaluation for baseline vs a configurable cache-admission gate.
#
# Goals:
# 1) Always evaluate from pristine source DB copies to avoid polluting source DBs
# 2) Run baseline and multiple admission thresholds under the same db_bench binary
...
ADMISSION_MODE="${ADMISSION_MODE:-ml}"
ADMISSION_TAG="${ADMISSION_TAG:-$ADMISSION_MODE}"
...
COLLECT_ADMISSION_SNAPSHOT="${COLLECT_ADMISSION_SNAPSHOT:-$COLLECT_ML_SNAPSHOT}"
...
validate_admission_mode() {
  case "$1" in
    ml|rf_hot) ;;
    *)
      echo "[ERR] unsupported ADMISSION_MODE: $1 (expected: ml | rf_hot)" >&2
      exit 1
      ;;
  esac
}
  ...
  if [[ "$variant" != "baseline" ]]; then
    case "$ADMISSION_MODE" in
      ml)
        ...
        ;;
      rf_hot)
        args+=(--enable_rf_hot_cache_admission=1)
        args+=(--rf_hot_cache_admission_threshold="$threshold")
        ;;
    esac
    if [[ "$COLLECT_ADMISSION_SNAPSHOT" == "1" ]]; then
...
echo "[INFO] ADMISSION_MODE=$ADMISSION_MODE ADMISSION_TAG=$ADMISSION_TAG"
echo "[INFO] THREADS=$THREADS READ_ONLY_DURATION=$READ_ONLY_DURATION MIXED_RW_DURATION=$MIXED_RW_DURATION"
echo "[INFO] PRESERVE_RUN_DB=$PRESERVE_RUN_DB COLLECT_ADMISSION_SNAPSHOT=$COLLECT_ADMISSION_SNAPSHOT"
    ...
    for cache_size in "${CACHE_SIZES[@]}"; do
      for seed in "${SEEDS[@]}"; do
        ...
        for threshold in "${THRESHOLDS[@]}"; do
          variant="${ADMISSION_TAG}_t${threshold}"
...
python - "$RUN_MANIFEST_CSV" "$RAW_RESULTS_CSV" "$COMPARE_CSV" "$THRESHOLD_SUMMARY_CSV" "$WORKLOAD_THRESHOLD_SUMMARY_CSV" "$REPORT_MD" "$ADMISSION_MODE" "$ADMISSION_TAG" <<'PY'
import csv
import math
import re
import sys
...
admission = raw[raw["variant"] != "baseline"].copy()
compare = admission.merge(
    baseline,
...
with report_md.open("w", encoding="utf-8") as f:
    ...
    f.write(f"- admission_runs: `{len(admission)}`\n")
    f.write(f"- admission_mode: `{admission_mode}`\n")
    f.write(f"- admission_tag: `{admission_tag}`\n")
...
echo "[DONE] online evaluation matrix complete: $OUT_ROOT (mode=$ADMISSION_MODE)"
